...
class CopyPaste(BaseMixTransform):
    ...
    Now supports `paste_in` mode, where objects are pasted into the same image instead of mixing different images.
    """

    def __init__(self, dataset=None, pre_transform=None, p=0.5, mode="flip") -> None:
        """Initializes CopyPaste object with dataset, pre_transform, and probability of applying Copy-Paste."""
        super().__init__(dataset=dataset, pre_transform=pre_transform, p=p)
        assert mode in {"flip", "mixup", "paste_in"}, f"Expected `mode` to be `flip`, `mixup`, or `paste_in`, but got {mode}."
        self.mode = mode
    ...
    def _mix_transform(self, labels):
        """Applies Copy-Paste augmentation with different modes."""
        if self.mode == "paste_in":
            return self._paste_in_transform(labels)
        labels2 = labels["mix_labels"][0]
        return self._transform(labels, labels2)

    def _paste_in_transform(self, labels):
        """
        Implements `paste_in` mode where objects from the same image are pasted at different locations.
        """
        im = labels["img"]
        cls = labels["cls"]
        h, w = im.shape[:2]
        ...
        # Generate random new locations for pasted objects
        for i in range(len(instances2)):
            x1, y1, x2, y2 = instances2.bboxes[i]
            obj_patch = im[y1:y2, x1:x2].copy()  # Copy the object region

            # Generate random new position
            new_x1 = np.random.randint(0, w - (x2 - x1))
            new_y1 = np.random.randint(0, h - (y2 - y1))
            new_x2 = new_x1 + (x2 - x1)
            new_y2 = new_y1 + (y2 - y1)

            # Paste the object in a new position
            im[new_y1:new_y2, new_x1:new_x2] = obj_patch

            # Update the bounding box coordinates
            instances2.bboxes[i] = [new_x1, new_y1, new_x2, new_y2]

        labels["img"] = im
        labels["cls"] = cls
        labels["instances"] = instances2
        return labels
    ...
    def __call__(self, labels):
        ...
        # Get images information that will be used for Mosaic or MixUp
        mix_labels = [self.dataset.get_image_and_label(i) for i in indexes]
